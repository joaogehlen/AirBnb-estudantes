...
import { MOCK_PROPERTIES, MOCK_HOST_BOOKINGS } from '../../lib/mockData';
...
export default function HostDashboardScreen() {
  ...
  const myProperties = MOCK_PROPERTIES.filter((p) => p.host_id === 'h1').slice(0, 2);
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.stats}>
          {[
            { label: 'Imóveis ativos', value: myProperties.length, icon: 'home-outline', color: COLORS.primary },
            { label: 'Pendentes', value: pendingCount, icon: 'time-outline', color: COLORS.warning },
            { label: 'Confirmadas', value: bookings.filter((b) => b.status === 'confirmed').length, icon: 'checkmark-circle-outline', color: COLORS.success },
          ].map((s) => (
              ...
              <View key={booking.id} style={styles.bookingCard}>
                ...
                {booking.status === 'pending' && (
                  <View style={styles.bookingActions}>
                    <Button label="Recusar" onPress={() => handleReject(booking.id)} variant="outline" size="sm" style={{ flex: 1 }} accessibilityLabel="Recusar reserva" />
                    <Button label="Aceitar" onPress={() => handleAccept(booking.id)} size="sm" style={{ flex: 1 }} accessibilityLabel="Aceitar reserva" />
                  </View>
                )}
              </View>
            );
